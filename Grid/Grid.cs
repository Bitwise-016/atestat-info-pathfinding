namespace Grid
{
    ...
    public class Grid
    {
        ...
        public Grid(int CelOrizontale, int CelVerticale)
        {
            _grid = new Cell[CelOrizontale, CelVerticale];
            for (var x = 0; x < _grid.GetLength(0); x++)
            {
                for (var y = 0; y < _grid.GetLength(1); y++)
                {
                    SetCell(x, y, CellType.Liber);
                }
            }
        ...
        public void Randomize(int seed)
        {
            ...
            // Trece prin toata matricea
            for (var x = 0; x < _grid.GetLength(0); x++)
            {
                for (var y = 0; y < _grid.GetLength(1); y++)
                {
                    // Umple casetele cu valori de 1 si 0 aleatoriu
                    _grid[x, y].Type = rand.Next(0, 10) > 5 ? CellType.Solid : CellType.Liber;
                    if (_grid[x, y].Type != CellType.Liber) continue;

                    //Daca este liber nodul, adauga un cost aleatoriu acestuia
